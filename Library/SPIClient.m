...
- (instancetype)init {
    ...
    if (self) {
        ...
        //   _tryToReconnectTimer           = [[SPIGCDTimer alloc]
        //   initWithObject:self queue:"com.assemblypayments.tryToReconnect"];
...
- (BOOL)start {
    ...
    if (self.posId) {
        ...
        self.spiMessageStamp =
        [[SPIMessageStamp alloc] initWithPosId:self.posId
...
- (void)initiatePurchaseTx:(NSString *)posRefId
    ...
    if (tipAmount > 0 && (cashoutAmount > 0 || promptForCashout)) {
        completion([[SPIInitiateTxResult alloc]
                    initWithTxResult:NO
                    message:
                    @"Cannot Accept Tips and Cashout at the same time."]);
    }
    __weak __typeof(&*self) weakSelf = self;
...
- (void)submitAuthCode:(NSString *)authCode
    ...
    dispatch_async(self.queue, ^{
        ...
        [weakSelf.state.txFlowState
         authCodeSent:[NSString stringWithFormat:@"Submitting Auth Code %@",
                       authCode]];
...
- (void)cancelTransaction {
    ...
    dispatch_async(self.queue, ^{
        ...
        if (weakSelf.state.flow != SPIFlowTransaction ||
            weakSelf.state.txFlowState.isFinished) {
            SPILog(@"Asked to cancel transaction but I was not in the middle of "
                   @"one.");
            return;
        }
        ...
        if (weakSelf.state.txFlowState.isRequestSent) {
            ...
            [weakSelf.state.txFlowState failed:nil
                                           msg:@"Transaction cancelled. Request "
             @"had not even been sent yet."];
        }
...
- (void)initiateSettleTx:(NSString *)pid
    ...
    dispatch_async(self.queue, ^{
        ...
        weakSelf.state.txFlowState = [[SPITransactionFlowState alloc]
                                      ...
                                      msg:@"Waiting for EFTPOS connection to make a settle "
                                      @"request"];
    ...
    dispatch_async(self.queue, ^{
        ...
        @synchronized(self) {
            ...
            self.state.txFlowState = [[SPITransactionFlowState alloc]
                                      ...
                                      amountCents:0
                                      message:stlEnqMsg
                                      msg:@"Waiting for EFTPOS connection to make a settlement "
                                      @"enquiry"];
...
- (void)initiateGetLastTxWithCompletion:(SPICompletionTxResult)completion {
    ...
    dispatch_async(self.queue, ^{
        ...
        weakSelf.state.txFlowState = [[SPITransactionFlowState alloc]
                                      ...
                                      amountCents:0
                                      message:gltMessage
                                      msg:@"Waiting for EFTPOS connection to make a get last "
                                      @"transaction request"];
...
- (void)connect {
    ...
    if (!self.secrets) {
        SPILog(@"Select the \"Pair with POS\" option on the EFTPOS terminal "
               @"screen. Once selected, the EFTPOS terminal screen should read "
               @"\"Start POS Pairing\"");
    }
...
- (void)handleKeyRequest:(SPIMessage *)m {
    ...
    SPISecretsAndKeyResponse *result = [SPIPairingHelper
                                        generateSecretsAndKeyResponseForKeyRequest:[[SPIKeyRequest alloc]
                                                                                    initWithMessage:m]];
    self.secrets = result.secrets; // we now have secrets, although pairing is
    // not fully finished yet.
    self.spiMessageStamp.secrets =
    self.secrets; // updating our stamp with the secrets so can encrypt
    // messages later.
    [self send:[result.keyResponse toMessage]]; // send the key_response, i.e.
    // interaction 3 of pairing.
}
...
- (void)handleKeyCheck:(SPIMessage *)m {
    ...
    self.state.pairingFlowState.message = [NSString
                                           stringWithFormat:@"Confirm that the following Code:\n\n%@\n\n is "
...
- (void)handlePairResponse:(SPIMessage *)m {
    ...
    if (pairResponse.isSuccess) {
        if (self.state.pairingFlowState.isAwaitingCheckFromPos) {
            SPILog(@"Got Pair Confirm from Eftpos, but still waiting for use "
                   @"to confirm from POS.");
            ...
            SPILog(@"Got Pair Confirm from Eftpos, and already had confirm "
                   @"from POS. Now just waiting for first pong.");
...
- (void)handleMotoPurchaseResponse:(SPIMessage *)m {
    ...
    dispatch_async(self.queue, ^{
        ...
        if (weakSelf.state.flow != SPIFlowTransaction ||
            ...
            ![weakSelf.state.txFlowState.posRefId
              isEqualToString:incomingPosRefId]) {
                SPILog(@"Received Cashout Response but I was not waiting for one. "
                       @"Incoming Pos Ref ID: %@",
                       incomingPosRefId);
                return;
            }
        ...
        if (!gltResponse.wasRetrievedSuccessfully) {
            if ([gltResponse isStillInProgress:txState.posRefId]) {
                ...
                if ([gltResponse isWaitingForSignatureResponse] && !txState.isAwaitingSignatureCheck) {
                    SPILog(@"Eftpos is waiting for us to send it signature "
                    ...
                    SPILog(@"Eftpos is waiting for us to send it auth code, but "
                    ...
                    [weakSelf.state.txFlowState
                     phoneForAuthRequired:[[SPIPhoneForAuthRequired alloc]
                                           ...
                                           phoneNumber:@"UNKNOWN"
                                           merchantId:@"UNKNOWN"]
                     msg:@"Recovered mid Phone-For-Auth but "
                     @"don't have details. You may "
                     @"Cancel then Retry."];
                } else {
                ...
                SPILog(@"Unexpected Response in Get Last Transaction during - Received posRefId:%@ error: %@",
                       [gltResponse getPosRefId], m.error);
    ...
    dispatch_async(self.queue, ^{
        ...
        if (weakSelf.state.flow == SPIFlowTransaction && !txState.isFinished) {
            ...
            if (state.isAttemptingToCancel &&
                ...
                [txState unknownCompleted:@"Waited long enough for cancel "
                 @"transaction result. Check EFTPOS. "];
                needsPublishing = YES;
                ...
                if (weakSelf.state.status != SPIStatusUnpaired) {
                    ...
                    @synchronized(self) {
                        if (weakSelf.state.flow == SPIFlowTransaction &&
                            ...
                            // TH-1D
                            SPILog(
                                   @"Lost connection in the middle of a transaction...");
                        }
                    }
                    ...
                    weakSelf.state.pairingFlowState.message =
                    @"Could not connect to pair. Check network/EFTPOS and try "
                    @"again...";
...
- (void)startPeriodicPing {
    ...
    _pingTimer =
    [[SPIGCDTimer alloc] initWithObject:self
                                  queue:"com.assemblypayments.ping"];
    
    [_pingTimer
     afterDelay:0
     repeat:true
     block:^(id self) {
         if (!weakSelf.connection.isConnected ||
             weakSelf.secrets == nil) {
         ...
         if (weakSelf.mostRecentPingSent != nil &&
             (weakSelf.mostRecentPongReceived == nil ||
              weakSelf.mostRecentPongReceived.mid !=
              weakSelf.mostRecentPingSent.mid)) {
                 weakSelf.missedPongsCount += 1;
                 SPILog(@"Eftpos didn't reply to my Ping. Missed Count: %i",
                        weakSelf.missedPongsCount / missedPongsToDisconnect);
...
- (void)onReadyToTransact {
    ...
    dispatch_async(self.queue, ^{
        if (self.state.flow == SPIFlowTransaction &&
            ...
            NSLog(@"onReadyToTransact, we were NOT in the middle of a "
...
- (void)handleIncomingPong:(SPIMessage *)m {
    ...
    if (self.mostRecentPongReceived == nil) {
        ...
        if (_state.status != SPIStatusUnpaired) {
            ...
            SPILog(@"First pong of connection but pairing process not "
                   @"finalised yet.");
        }
    }
...
- (void)handleSettlementEnquiryResponse:(SPIMessage *)m {
    ...
    dispatch_async(self.queue, ^{
        if (weakSelf.state.flow != SPIFlowTransaction ||
            weakSelf.state.txFlowState.isFinished) {
            SPILog(@"Received Settlement Enquiry response but I was not waiting "
                   @"for one. %@",
                   [m decryptedJson]);
            return;
        }
...
- (void)onSpiMessageReceived:(NSString *)message {
    ...
    dispatch_async(self.queue, ^{
        ...
        if ([eventName isEqualToString:SPIKeyRequestKey]) {
            ...
            SPILog(@"I could not verify message from EFTPOS. You might have to "
                   @"un-pair EFTPOS and then reconnect.");
            ...
            SPILog(@"I don't understand event:'%@', %@. Perhaps I have not "
                   @"implemented it yet.",
                   m.eventName, m.data);
        }
    });
